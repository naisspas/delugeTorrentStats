			...
			array(
				...
				'next'		=> array(
					array(
						'parameter' => 'action',
						'value'		=> 'list',
						'presence'	=> array('existingTorrentOnly')
					)
				)
			)			
		)	
	...
	if($exceptions->getCount()==0){
		...
		switch($forward->path){
			...
			case 'torrent:list': 
				...
				$outputData['toDate'] = $toDate;
				$outputData['rows'] = $db->getList(
					$db->getBooleanForm($request->get('existingTorrentOnly')),
					is_object($fromDate) ? NULL : $fromDate,
					is_object($toDate) ? NULL : $toDate
				);
				break;
			case 'config:update':
